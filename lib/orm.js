  ...
  getCollection(function (err, coll) {
    ...
    coll.find({ _id: _id }, function (err, cursor) {
      ...
      cursor.toArray(async function (err, docs) {
        ...
        console.log(`happyDelay`)
        await delay(1000)
        const digestMongo = digest(docs[0])
        console.log(`happyGet ${Object.keys(docs[0])} ${digestMongo}`)
...
exports.add = function (stamp, value, cb) { // cb(err,doc)
  ...
  if (!stamp) {
    ...
    if (isNaN(unixStamp)) {
      ...
      if (cb) cb({ message: `Invalid Date: ${stamp}` });
      return;
    }
  ...
  if (isNaN(asnum)) {
    ...
    if (cb) cb({ message: `Invalid Value: ${value}` });
    return;
  }
